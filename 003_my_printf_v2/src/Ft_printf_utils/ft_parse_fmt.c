...
int	ft_parse_fmt(t_data *data_s)
{
	...
	data_s->fmt_flags_s.precision_val = -1;
		// Because we will consider (.0) as a case so we check if precision is major  or equal than 0; Not the width; (WIDTH CANT == 0) but precision can.
