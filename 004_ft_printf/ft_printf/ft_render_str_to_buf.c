...
static	void	ft_add_n_of_str_into_bfr(t_data_s *data_s, int len, char *str_arg)
{
	if (len <= 0)
		return ;
	while (len-- && *str_arg)
		ft_add_n_chrs_into_bfr(data_s, 1, *str_arg++);
...
void	ft_render_str_to_buf(t_data_s *data_s, char *str_arg)
{
	...
	{
		if (data_s->flags_s.precision_val >= 0)
			ft_add_n_of_str_into_bfr(data_s, data_s->flags_s.precision_val, str_arg);
		else
